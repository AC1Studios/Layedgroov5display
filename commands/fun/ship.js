...
module.exports = class ShipCommand extends Command {
  ...
  async run(msg, { first, second }) {
    ...
    const authorUser =
      first.id === msg.author.id || second.id === msg.author.id;
    const botUser =
      first.id === this.client.user.id || second.id === this.client.user.id;
    const girlfriendUser = first.id === config.GF || second.id === config.GF;
    const friendUser = first.id === config.GF || second.id === config.GF;
    ...
    if (owner && botUser) {
      ...
      level = 100;
    } else if (friendUser && owner) {
      level = 100;
    } else {
      const calculated = Math.abs(
        Number.parseInt(BigInt(first.id) - BigInt(second.id), 10)
      );
    ...
    const secondAvatarURL = second.displayAvatarURL({
      format: "png",
      size: 512
    });
    try {
      ...
      ctx.fillText("~layedgroov's Compatability Meter~", 600, 15);
      ctx.fillStyle = "white";
      ...
      ctx.fillText(
        this.calculateLevelText(level, self, owner, authorUser, botUser),
        600,
        296
      );
