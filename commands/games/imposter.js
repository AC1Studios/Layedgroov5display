...
module.exports = class ImposterCommand extends Command {
    ...
    async run(msg, { playersCount }) {
        ...
        try {
            ...
            const imposter =
                awaitedPlayers[
                    Math.floor(Math.random() * awaitedPlayers.length)
                ];
            ...
            while (players.filter(player => !player.killed).size > 2) {
                const playersLeft = players.filter(player => !player.killed)
                    .size;
                await msg.say(
                    `There are **${playersLeft}** players left. Talk until someone says the kill word.`
                );
                const filter = res => {
                ...
                if (msgs.size) {
                    ...
                    try {
                        ...
                        await msg.say(`${killedMsg.author} has been killed`);
                    } catch {
                    ...
                    await msg.say(stripIndents`
						...
						Talk amongst yourselves, who is the imposter?.
					`);
                } else {
                    await msg.say(stripIndents`
                ...
                let i = 0;
                await msg.say(stripIndents`
					Alright, who do you think the imposter is?.
                ...
                const voteFilter = res => {
                    ...
                    if (
                        int >= 1 &&
                        int <= players.filter(p => !p.killed).size
                    ) {
                ...
                const kicked = players.get(
                    votes.sort((a, b) => b.votes - a.votes).first().id
                );
                ...
                if (amountLeft.size > 2) {
                    ...
                    winners.push(
                        players.find(player => player.imposter).user.tag
                    );
                    break;
                }
            }
            this.fighting.delete(msg.channel.id);
            return msg.say(
                `Congrats, ${list(winners)}! The kill word was **${word}**.`
            );
        } catch (err) {
            this.fighting.delete(msg.channel.id);
            throw err;
